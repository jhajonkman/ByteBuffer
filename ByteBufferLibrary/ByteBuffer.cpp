...
int ByteBuffer::putU16InFront(uint16_t in){
    byte *pointer = (byte *)&in;
	putInFront(pointer[0]);	
	putInFront(pointer[1]);	
}

int ByteBuffer::putU16(uint16_t in){
    byte *pointer = (byte *)&in;
	put(pointer[1]);	
	put(pointer[0]);	
}
...
uint16_t ByteBuffer::getU16FromBack(){
	uint16_t ret;
    byte *pointer = (byte *)&ret;
...
int ByteBuffer::putTimeInFront(time_t in){
    byte *pointer = (byte *)&in;
	putInFront(pointer[0]);	
	putInFront(pointer[1]);	
	putInFront(pointer[2]);	
...
int ByteBuffer::putTime(time_t in){
    byte *pointer = (byte *)&in;
	put(pointer[3]);	
	put(pointer[2]);	
	put(pointer[1]);	
...
time_t ByteBuffer::getTimeFromBack(){
	time_t ret;
    byte *pointer = (byte *)&ret;
	pointer[0] = getFromBack();
	pointer[1] = getFromBack();
